...
def iterate(p, d, target, tol, history):
    b = np.copy(p[0]) # prevent b changed
...
def plotIterations(history, p, save=False): # zeros()의 shape를 결정지을 수 없으므로 모든 경로를 그리지 말고 initial state와 final state만 그리기
    ...
    if save == True:
            plt.savefig('{}.png'.format("my_fabrik_3D"))
...
p = np.zeros((totalLinks, 3))
d = np.zeros(totalLinks-1)
History = np.zeros((2, totalLinks, 3))
...
Target = np.array([Target_[0], Target_[1], Target_[2]])
